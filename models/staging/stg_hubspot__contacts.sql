...
renamed as (
    select
        ...
        regexp_extract(lower(property_email), r'@([^@]+)$')        as email_domain,
        ...
        safe_cast(property_numberofemployees as int64)             as company_size_employees,
        safe_cast(property_annualrevenue as numeric)               as company_annual_revenue,
        lower(property_lifecyclestage)                             as lifecycle_stage,
        lower(property_hs_lead_status)                             as lead_status,
        safe_cast(property_createdate as timestamp)                as contact_created_at,
        safe_cast(property_hs_lifecyclestage_lead_date as timestamp)       as became_lead_at,
        safe_cast(property_hs_lifecyclestage_marketingqualifiedlead_date as timestamp) as became_mql_at,
        safe_cast(property_hs_lifecyclestage_salesqualifiedlead_date as timestamp)     as became_sql_at,
        safe_cast(property_hs_lifecyclestage_opportunity_date as timestamp)            as became_opportunity_at,
        safe_cast(property_hs_lifecyclestage_customer_date as timestamp)               as became_customer_at,
